        ...
        while bot.is_connected:
            ...
            if chat_msg.startswith('/'):
                ...
                if cmd == '/q':
                    ...
                    if len(bot.users.nli) is 0:
                        print ('all users logged in.')
                    else:
                        bot.do_purge()

                elif cmd == '/status':
                    if pinybot.pinylib.CONFIG.B_ALLOW_GUESTS:
                        gs = 'down'
                    else:
                        gs = 'up'
                    bot.console_write(pinybot.pinylib.COLOR['wgate'], '-] the gate is ' + gs + ' [-')
                    modz = str(len(bot.users.mods))
                    normz = str(len(bot.users.norms))
                    bot.console_write(pinybot.pinylib.COLOR['wgate'], '-] there are ' + modz + ' mods in and ' + normz + ' users in [-')
                    ...
                    pinybot.pinylib.config.B_FORGIVE_AUTO_BANS = not pinybot.pinylib.config.B_FORGIVE_AUTO_BANS
                    ...
                    if len(msg_parts) is 2:
                        _user = msg_parts[1]
                        bot.do_bad_account(_user)

                elif cmd == '/rmba':
                    if len(msg_parts) is 2:
                        _user = msg_parts[1]
                        bot.do_remove_bad_account(_user)
