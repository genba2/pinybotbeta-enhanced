...
from util import core, string_util, file_handler, unicode_catalog
...
COLOR = {
    ...
    'wgate': Back.WHITE + Fore.BLACK + Style.BRIGHT,
    'ugate': Back.BLUE + Fore.BLACK + Style.BRIGHT
}
...
class TinychatRTMPClient:
    ...
    def on_joins(self, joins_info_dict):
        ...
        CONFIG.B_ULTRAS = file_handler.file_reader(config.CONFIG_PATH, CONFIG.B_ULTRAS_FILE_NAME)
        if _user is not None:
            if _user.account:
                if _user.is_owner:
                    _user.user_level = 1
                    ...
                    self.console_write(COLOR['bright_red'], 'Joins Moderator %s:%d:%s' %
                                       (_user.nick, _user.id, _user.account))
                    if _user.account in CONFIG.B_ULTRAS:
                        _user.user_level = 2
                else:
                    _user.user_level = 5
    ...
    def on_quit(self, uid, name):
        ...
        if self.users.delete(name):
            self.console_write(COLOR['dcyan'], '%s:%s left the room.' % (name, uid))
        else:
    ...
    def on_from_owner(self, owner_msg):
        ...
        #msg = str(msg).replace(unicode_catalog.scircle, 'gk')
        #msg = str(msg).replace(unicode_catalog.ssquare, 'tw')
