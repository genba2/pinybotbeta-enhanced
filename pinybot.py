...
class TinychatBot(pinylib.TinychatRTMPClient):
    ...
    def on_avon(self, uid, name):
        ...
        :param name: str
        """
        if not pinylib.CONFIG.B_ALLOW_GUESTS and self._is_client_mod:
            hoe = self.users.search(name)
            if not hoe.account:
                self.send_ban_msg(name, uid)
                if pinylib.CONFIG.B_FORGIVE_AUTO_BANS:
                    self.send_forgive_msg(uid)

        if pinylib.CONFIG.B_SCREEN and self._is_client_mod:
            hoe = self.users.search(name)
            if not hoe.account:
                if not hoe.screened:
                    ...
                    hoe.screened = True
                    #zz4
                    self.send_private_msg('You must be logged in to a tinychat account to broadcast in this room.', name)
                else:
                    self.send_ban_msg(name, uid)
                    if pinylib.CONFIG.B_FORGIVE_AUTO_BANS:
                        self.send_forgive_msg(uid)
    ...
    def on_nick(self, old, new, uid):
        ...
        if self.check_nick(old, old_info):
            ...
            #derp = self.users.
            if old_info.account and old_info.account in pinylib.CONFIG.B_ACCOUNT_BANS:
                if self._is_client_mod:
                    sleep(0.1)
                    self.send_ban_msg(old_info.nick, old_info.id)
                    if pinylib.CONFIG.B_FORGIVE_AUTO_BANS:
                        self.send_forgive_msg(old_info.id)
    ...
    def message_handler(self, decoded_msg):
        ...
        if decoded_msg.startswith(prefix):
            ...
            if self.has_level(2):
                ...
                elif cmd == prefix + 'bafn':
                    self.do_bad_account_from_nick(cmd_arg)
            ...
            if self.has_level(3):
                ...
                elif cmd == prefix + 'zdown':
                    self.do_cam_down()

                elif cmd == prefix + 'gate':
                    self.do_guests()

                elif cmd == prefix + 'screen':
                    self.do_screen()
    ...
    def do_purge(self):
        for hoecheck in self.users.nli:
            ...
            if pinylib.CONFIG.B_FORGIVE_AUTO_BANS:
                self.send_forgive_msg(hoecheck.id)
        self.send_bot_msg(unicode_catalog.scircle + ' *guest purge complete, thank you for being logged in*')

    def do_screen(self):
        pinylib.CONFIG.B_SCREEN = not pinylib.CONFIG.B_SCREEN
        if pinylib.CONFIG.B_SCREEN:
            ss = 'active'
        else:
            ss = 'inactive'
    ...
    def do_infoz(self):
        ...
        uptimez = self.format_time(self.get_runtime())
        self.console_write(pinylib.COLOR['ugate'], '[-] (o_o) (o_o) (o_o) [-]')
        self.console_write(pinylib.COLOR['ugate'], '[-] the gate is ' + gs)
    ...
    def do_kick(self, user_name):
        ...
        if self._is_client_mod:
            ...
            elif user_name in pinylib.CONFIG.B_ULTRAS:
                print ('--warning, attempt at !kick on protected user--')
            else:
                _user = self.users.search(user_name)
                if _user is None:
                    self.send_bot_msg(unicode_catalog.INDICATE + ' No user named: *%s*' % user_name)
    ...
    def do_ban(self, user_name):
        ...
        if self._is_client_mod:
            if len(user_name) is 0:
                ...
                print ('--warning, attempt at !ban on protected user--')
                self.send_bot_msg(unicode_catalog.scircle + ' *Command refused; user is protected.* ')
            else:
                _user = self.users.search(user_name)
                if _user is None:
                    self.send_bot_msg(unicode_catalog.INDICATE + ' No user named: *%s*' % user_name)
                    ...
                    self.send_bot_msg(unicode_catalog.INDICATE + ' Not allowed.')
                elif _user.account in pinylib.CONFIG.B_ULTRAS:
                    print ('--warning, attempt at !ban on protected user--')
                    self.send_bot_msg(unicode_catalog.scircle + ' *Command refused; user is protected.* ')
                else:
                    self.send_ban_msg(user_name, _user.id)
    ...
    def do_bad_nick(self, bad_nick):
        ...
        if self._is_client_mod:
            ...
            #zz2
            elif bad_nick in pinylib.CONFIG.B_ULTRAS:
                #print ('--warning, attempt at !bn on protected user--')
                self.send_bot_msg(unicode_catalog.scircle + ' *Command refused; user is protected.* ')
            else:
                _user = self.users.search(bad_nick)
                if _user:
                    if _user.account:
                        if _user.account in pinylib.CONFIG.B_ULTRAS:
                            #print ('--warning, attempt at !bn on protected user--')
                        ...
                        if pinylib.CONFIG.B_FORGIVE_AUTO_BANS:
                            self.send_forgive_msg(_user.id)
                pinylib.file_handler.file_writer(self.config_path(), pinylib.CONFIG.B_NICK_BANS_FILE_NAME, bad_nick)
                self.send_bot_msg(unicode_catalog.scircle + ' *%s* was added to file.' % bad_nick)
        ...
        if self._is_client_mod:
            if len(bad_nick) is 0:
                ...
                if bad_nick in pinylib.CONFIG.B_NICK_BANS:
                    ...
                    if rem:
                        self.send_bot_msg(unicode_catalog.scircle + ' *%s* was removed.' % bad_nick)
                        self.load_list(nicks=True)
    ...
    def do_bad_string(self, bad_string):
        ...
        if self._is_client_mod:
            ...
            #     self.send_bot_msg('Ban string to short: ' + str(len(bad_string)))
                ...
                self.send_bot_msg(unicode_catalog.scircle + ' *%s* was added to file.' % bad_string)
            ...
            if len(bad_string) is 0:
                ...
                if bad_string in pinylib.CONFIG.B_STRING_BANS:
                    rem = pinylib.file_handler.remove_from_file(self.config_path(),
                                                                pinylib.CONFIG.B_STRING_BANS_FILE_NAME, bad_string)
                    if rem:
                        self.send_bot_msg(unicode_catalog.scircle + ' *%s* was removed.' % bad_string)
                        self.load_list(strings=True)
    ...
    def do_bad_account(self, bad_account_name):
        ...
        if self._is_client_mod:
            ...
            #     self.send_bot_msg('Account to short: ' + str(len(bad_account_name)))
                ...
                self.send_bot_msg(unicode_catalog.scircle + ' %s is already in list.' % bad_account_name)
            elif bad_account_name in pinylib.CONFIG.B_ULTRAS:
                print ('--warning, attempt at !ba on protected user--')
                self.send_bot_msg(unicode_catalog.scircle + ' *Command refused; user is protected.* ')
            else:
                pinylib.file_handler.file_writer(self.config_path(),
                                                 pinylib.CONFIG.B_ACCOUNT_BANS_FILE_NAME, bad_account_name)
                self.send_bot_msg(unicode_catalog.scircle + ' *%s* was added to file.' % bad_account_name)
                self.load_list(accounts=True)
                #zz1
                for hoecheck in self.users.signed_in:
                    if hoecheck.account == bad_account_name:
                        self.send_ban_msg(hoecheck.nick, hoecheck.id)
                        if pinylib.CONFIG.B_FORGIVE_AUTO_BANS:
                            self.send_forgive_msg(hoecheck.id)
                        break
    def do_bad_account_from_nick(self, bad_nick):
        hoe = self.users.search(bad_nick)
        if not hoe:
            self.send_bot_msg(unicode_catalog.scircle + ' nick not found.')
        elif not hoe.account:
            self.send_bot_msg(unicode_catalog.scircle + ' account not found.')
        else:
            pinylib.file_handler.file_writer(self.config_path(),
                                                 pinylib.CONFIG.B_ACCOUNT_BANS_FILE_NAME, hoe.account)
            self.send_bot_msg(unicode_catalog.scircle + ' *%s* was added to file.' % hoe.account)
            self.load_list(accounts=True)
            self.send_ban_msg(hoe.nick, hoe.id)
            if pinylib.CONFIG.B_FORGIVE_AUTO_BANS:
                self.send_forgive_msg(hoe.id)
        ...
        if self._is_client_mod:
            if len(bad_account) is 0:
                ...
                if bad_account in pinylib.CONFIG.B_ACCOUNT_BANS:
                    ...
                    if rem:
                        self.send_bot_msg(unicode_catalog.scircle + ' *%s* was removed.' % bad_account)
                        self.load_list(accounts=True)
    ...
    def do_uptime(self):
        ...
        if pinylib.config.B_FORGIVE_AUTO_BANS:
            ...
            afbz = '0'
        self.send_bot_msg(unicode_catalog.scircle + '  ' + self.format_time(self.get_runtime()) + ' - gate ' + gs + ', screen ' + ss + ' - ' + afbz)
        self.console_write(pinylib.COLOR['wgate'], '-] the gate is ' + gs + ' [-')
    ...
    def private_message_handler(self, private_msg):
        ...
        if private_msg.startswith(prefix):
            ...
            if self.has_level(2):
                if pm_cmd == prefix + 'purge':
                    self.do_purge()

                elif pm_cmd == prefix + 'say':
                    self.send_chat_msg(pm_arg)

                elif pm_cmd == prefix + 'afb':
                    pinylib.config.B_FORGIVE_AUTO_BANS = not pinylib.config.B_FORGIVE_AUTO_BANS
            ...
            if self.has_level(3):
                ...
                elif pm_cmd == prefix + 'down':
                    self.do_cam_down()

                elif pm_cmd == prefix + 'gate':
                    self.do_guests()

                elif pm_cmd == prefix + 'screen':
                    self.do_screen()
        ...
        if self._client_id != user_info.id:
            if not pinylib.CONFIG.B_ALLOW_GUESTS and self._is_client_mod:
                if not user_info.account:
                    ...
                    if pinylib.CONFIG.B_FORGIVE_AUTO_BANS:
                        self.send_forgive_msg(user_info.id)
                    return True
    ...
    def sanitize_message(self, decoded_msg):
        ...
        if pinylib.CONFIG.B_UNICODE_SPAM:
            ...
            for spam_unicode in range(len(spam_characters)):
                if spam_characters[spam_unicode] in decoded_msg_pieces:
                    ...
                    self.console_write(pinylib.COLOR['wgate'], '-] unicode spam detected [-')
                    return True
            ...
            if 'tinychat.com/' + self.roomname not in decoded_msg:
                ...
                if msg_search is not None:
                    ...
                    if matched[0] is not None:
                        self.send_ban_msg(self.active_user.nick, self.active_user.id)
                        self.console_write(pinylib.COLOR['wgate'], '-] tinychat link spam detected [-')
        ...
        if pinylib.CONFIG.B_CAPITAL_CHARACTER_LIMIT:
            ...
            if over_limit:
                self.send_ban_msg(self.active_user.nick, self.active_user.id)
                self.console_write(pinylib.COLOR['wgate'], '-] caps spam detected lol [-')
                if pinylib.CONFIG.B_FORGIVE_AUTO_BANS:
                    self.send_forgive_msg(self.active_user.id)
                # self.send_bot_msg('*Ban this*.')
                return True
        ...
        if not pinylib.CONFIG.B_ALLOW_SNAPSHOTS:
            if self._snapshot_message in decoded_msg:
                self.send_ban_msg(self.active_user.nick, self.active_user.id)
                self.console_write(pinylib.COLOR['wgate'], '-] snapshot detected [-')
                if pinylib.CONFIG.B_FORGIVE_AUTO_BANS:
                    self.send_forgive_msg(self.active_user.id)
                # self.send_bot_msg('*Ban this*.')
                return True
    ...
    def do_auto_url(self, decoded_message):
        ...
        :param decoded_message: str the complete message sent by the user, which we will check for a URL.
        """
